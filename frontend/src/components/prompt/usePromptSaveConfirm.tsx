...
// publish 同款二次确认流：store.publishPromptTemplate 返回三态结果，本 hook 负责：
//   - needsConfirm（后端 200 needs_human_confirm）：弹框逐字核对 diff → 勾选后带 force 重提
//   - rejected（后端 4xx 红线语义审查拒绝）：弹框显拒绝理由 + 强制发布入口 → 带 force 重提
//   - ok / error：直接结束（error 已在 store 走 setError）
// 必须在 <ConfirmProvider> 子树内调用。
export function usePromptPublishConfirm() {
  const confirm = useConfirm();
  const publishPromptTemplate = useStrategyStore((s) => s.publishPromptTemplate);

  // 返回一个发布触发器：执行发布并按三态走二次确认（按钮传 id，非表单事件）。
  return async function runPublish(id: string): Promise<void> {
    let result: SavePromptResult = await publishPromptTemplate(id);
    if ("ok" in result || "error" in result) return;
    ...
    if ("needsConfirm" in result) {
      ...
      if (!ok) return;
      result = await publishPromptTemplate(id, true);
      // force 后若仍非 ok（error 已 setError），结束。
      return;
    }
    ...
    if ("rejected" in result) {
      ...
      if (!ok) return;
      await publishPromptTemplate(id, true);
    }
  };
}
