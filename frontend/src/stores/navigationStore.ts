 ...
 *  v2 曾短暂上线，所以它也必须进 LEGACY_KEYS 一起清掉。 */
const STORAGE_KEY = "wa.nav.collapsed.v3";
...
/** 首次进入时默认收起的组。
 *
 *  分组从 5 组变 6 组后，标签 + 组间距的固定开销涨了（6 个标签 × 23px + 5 个间距），
 *  可放行数上限降到 10。所以默认必须收起 3 组才不出滚动条：
 *    - 「平台配置」—— 账号/模型/策略/自治回路/演化，配好就不动
 *    - 「建设规划」—— 两个未上线占位频道，点不了
 *    - 「知识资产」—— 知识库/话术/名片，配一次用很久，不是每天改
 *  余下「客户运营」「决策审批」「运行监控」展开 = 10 行。
 *  实测（真实 Chrome）1440×900 下内容 592px、可用 611px，余 19px 不滚动。
 *
