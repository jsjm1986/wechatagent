// 配置段独立视图（Task 6 从 CockpitPanel 的 ConfigureContent 抽出，与 ObserveView.tsx 对称）。
// 抽文件是 verbatim 搬迁不是重写：JSX / 回调名 / memoryDraft·guidePreview 等数据引用一字不改，
// 只把同文件临时函数 ConfigureContent 落成独立文件，CockpitPanel 改 import 挂载。
// 配置段全部使用全局 styles.css 类名，不引用 cockpit.module.css（与原 ConfigureContent 一致）。
import {
  Activity,
  Bot,
  SendHorizonal,
