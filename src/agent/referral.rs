...
/// 措辞边界：用「专属顾问/增配/我仍在场辅助」等经审核的表达；最终语义与发送准入
/// 由独立审查和结构化授权围栏判断，不在这里维护关键词表。
/// assist 关账号不注入此段（reviewer system prompt 字节等价）。
pub(crate) const REVIEWER_ASSIST_YIELD_NOTE: &str = "\n\
【受控例外·辅助模式——必须按决策事实评审】本账号已开启辅助模式。\
决策事实面中的 namecardToSend 非空，表示本轮候选回复与发送该张管理员已配置名片的动作绑定；\
候选表示「我把这位专属顾问的名片发给你」或建议客户与其继续深入对接，是 AI 发起的受控业务动作，\
不属于把当前对话转交真人，也不违反 AI 自治红线。\
不得仅因出现第三方姓名、「专属顾问」、发送名片或建议后续对接，就降低 boundaryPrivacySafety、\
抬高 factRisk、标为产品知识不足或触发安全拦截；该受控引荐动作本身不是产品能力、价格或效果声明。\
若没有真实的内部信息泄露，\
boundaryPrivacySafety 应按普通安全回复评分（通常 8 分以上）。AI 仍需以自己的口吻继续承接答疑。\
此例外只授权 namecardToSend 对应的名片发送与引荐动作，不授权任何额外事实：\
若 namecardToSend 为空，或候选明确宣称真人将接管当前对话，或额外编造顾问资历、\
可用时间、合同步骤、所需材料、价格、服务效果等未获支持事实，仍应正常改写或拦截。\n";
...
mod tests {
    ...
    fn reviewer_assist_yield_note_covers_two_paths_and_passes_lint() {
        ...
        // 具体措辞由独立语义审查和发送围栏统一把关；本模块只负责受控引荐语义，
        // 不复制任何禁词字面量。本测试只断言两路径语义。
    }
