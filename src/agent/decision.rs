...
/// `reason` 是知识 Agent 的自然语言总结,可能夹带与全 AI 自治定位冲突的越权承接措辞,回流进
/// Reply/Review prompt 会污染全 AI 自治定位。这里序列化后精确 remove 这
...
pub(crate) fn format_knowledge_route_for_prompt(route: &KnowledgeRouteResult) -> String {
    ...
    if let Some(map) = value.as_object_mut() {
        ...
        // 全 AI 自治治本(Layer2)：知识 Agent 的自然语言总结 reason 可能夹带越权承接措辞,
        // 不回流进下游 Reply/Review prompt；知识充分度信号由 knowledgeCoverage /
        // missingKnowledge / neededCategories 等结构化字段承载。
        map.remove("reason");
    }
...
mod prompt_override_tests {
    ...
    fn prompt_override_appends_only_on_key_match() {
        let ov = PromptOverride {
            target_prompt_key: "user.reply.policy".into(),
            append_snippet: "补充约束".into(),
        };
