...
- query 同时包含“用户当前消息”和“最近对话”时，用户当前消息是本轮唯一主查询，最近对话只提供人物与主题连续性。先按完整语义判断当前消息是否确实继续、确认、修改或追问某个历史事项；只有成立时才把旧的未完成任务、预约、请示或知识缺口带入本轮。若当前消息是在完成一个独立的社交或关系性会话行为，且没有延续旧事项，应直接用 not_required + none + respond，不得因历史里存在开环任务而将其复活。该判断不得退化为问候词、短语或其它客户文本匹配。
...
- recommendedNextStep=clarify_customer 时，clarificationQuestion 必须是一个简短、自然、可直接问客户的问题，只补最关键的一项 missingInformation，不预设缺失事实已经成立，不包含内部角色、路由或控制字段；其它 nextStep 时 clarificationQuestion 留空。它只是给下游 Reply/Reviewer 的 AI 提议，仍须经过完整授权链路。
...
mod tests {
    ...
    #[test]
    fn action_protocol_prioritizes_current_message_without_customer_phrase_rules() {
        assert!(ACTION_PROTOCOL.contains("用户当前消息是本轮唯一主查询"));
        assert!(ACTION_PROTOCOL.contains("最近对话只提供人物与主题连续性"));
        assert!(ACTION_PROTOCOL.contains("不得因历史里存在开环任务而将其复活"));
        assert!(ACTION_PROTOCOL.contains("not_required + none + respond"));
        assert!(ACTION_PROTOCOL.contains("不得退化为问候词、短语"));
        assert!(ACTION_PROTOCOL.contains("clarificationQuestion 必须是一个简短、自然"));
        assert!(!ACTION_PROTOCOL.contains("你好/在吗"));
    }
    ...
    fn parse_answer_action_with_camel_alias() {
        ...
        match action {
            AgentAction::Answer {
                ...
                assert!(resolution.clarification_question.is_empty());
            }
    ...
    #[test]
    fn parse_answer_action_preserves_structured_clarification_question() {
        let raw = json!({
            "action": "answer",
            "citedChunkIds": [],
            "sourceQuotes": [],
            "answer": "More customer context is required.",
            "resolution": {
                "answerability": "unsupported",
                "requiredAuthority": "customer",
                "recommendedNextStep": "clarify_customer",
                "missingInformation": ["the preferred delivery address"],
                "clarificationQuestion": "Which delivery address should I use?",
                "authorityQuestion": "",
        ...
        match action {
            AgentAction::Answer { resolution, .. } => assert_eq!(
