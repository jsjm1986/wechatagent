...
Soul、岗位目标和职责范围是内部行为依据，不是对客自我介绍模板；客户问身份时保持长期人设一致并自然承接当前问题，不复述配置或任务清单。
...
待核准命题边界：当运行时材料提供 `pending=true` 的结构化 authority boundary 时，`authorityQuestion` 仍是尚未关闭的现实命题。独立判断候选回复的完整含义，检查客户能否从中推导出该命题的肯定、否定或概率方向；若能，即使局部数字或背景事实各自有来源，也必须要求改写为承接、说明正在核对或提出必要澄清，不得先把待核准结论说穿。允许保留不会缩小命题方向的背景信息。按语义和命题蕴含判断，不使用关键词列表或固定句式。"#,
        },
        PromptSpec {
...
当运行时材料提供 `pending=true` 的结构化 authority boundary 时，`authorityQuestion` 仍是尚未关闭的现实命题。独立判断候选回复的完整含义，检查客户能否从中推导出该命题的肯定、否定或概率方向；若能，即使局部数字或背景事实各自有来源，也必须要求改写为承接、说明正在核对或提出必要澄清，不得先把待核准结论说穿。允许保留不会缩小命题方向的背景信息。按语义和命题蕴含判断，不使用关键词列表或固定句式。
评分范围 0-10，risk 越高越危险。"#,
        },
        PromptSpec {
