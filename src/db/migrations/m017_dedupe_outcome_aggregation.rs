//! 2026_05_X1_002（P1-1 配套）：在新增 outcome aggregation partial unique index
...
//! 导致 `(workspace_id, kind, account_id, content)` 出现多条记录。新索引带 partial filter
//! `kind="outcome_aggregation"` 限定范围，但首启时若已有重复行，索引创建会失败。
//!
//! 策略：保留每组 `(workspace_id, account_id, content)` 中 `created_at` 最小（最早）的一条，
...
pub(super) async fn run_step(db: &Database) -> AppResult<()> {
    ...
    let mut cursor = db.tasks().aggregate(dedupe_pipeline(), None).await?;
...
fn dedupe_pipeline() -> Vec<Document> {
    vec![
...
mod tests {
    ...
    #[test]
    fn dedupe_group_is_workspace_scoped() {
        let pipeline = dedupe_pipeline();
