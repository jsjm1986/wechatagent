...
// ── 结果三态分类器（H1 上移）─────────────────────────────────────────────
//
// 优化第五波·线 H：`classify_outcome_label` / `classify_outcome_label_with_polarity`
// / `OutcomeLabel` / DEFAULT 极性常量 / `resolve_effective_polarity` 的实现已逐字
// 上移到 [`crate::agent::outcome_label`]（单一真相源，演化器 significance /
// post_release 与回路① 共享）。本模块保留同可见性 re-export，旧引用路径
// （`crate::knowledge_wiki::gap_signals::classify_outcome_label` 等）行为逐字节等价，
// 下方测试原地不动继续锁旧路径行为。
pub use crate::agent::outcome_label::{
    classify_outcome_label, classify_outcome_label_with_polarity, OutcomeLabel,
};
pub(crate) use crate::agent::outcome_label::{
...
// 2.5-main-2 的 `resolve_effective_polarity` 同样上移到 `crate::agent::outcome_label`
// （H1 单一真相源）；同可见性 re-export，调用方（本模块 / lessons_learned /
// evolution::post_release）路径不变。
pub(crate) use crate::agent::outcome_label::resolve_effective_polarity;
