...
def metric(label, value):
    ...
    return (
        ...
        f"p50={value['p50']} p95={value['p95']} p99={value['p99']} max={value['max']}"
    )


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--base-url", default=os.getenv("WECHATAGENT_BASE_URL", "http://127.0.0.1:3000"))
    parser.add_argument("--hours", type=int, default=24)
    parser.add_argument("--account-id")
    parser.add_argument("--path", choices=["direct", "escalated", "rewrite", "revision", "no_reply", "manual"])
    parser.add_argument("--session", default=os.getenv("WA_SESSION"), help="wa_session cookie value")
    ...
    url = args.base_url.rstrip("/") + "/api/admin/observability/performance?" + urllib.parse.urlencode(query)
    ...
    print(f"Gateway performance: {body['windowStart']} -> {body['asOf']} truncated={body['truncated']}")
