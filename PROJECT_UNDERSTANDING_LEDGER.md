...
**main CI 红灯波（2026-08-14 深夜，首轮有效 main CI 两 job 红，当夜修复）**：① Baseline gate 红：`check-secrets.py` HC-001 `rotated-secret-provider-binding` 规则拦下线 C quality-gold job 的 `${{ vars.* }}` 动态 provider 引用（该 job 注释自预言"或合并时按字面值内联"但未做）——按 real-llm job 同款字面值内联 `REAL_LLM_BASE_URL/MODEL` 修复，secret-scan 归零。② Integration 红（三个 knowledge 测试）：**28 号 §4#17 预言精确兑现**（"sr132 按现行口径锁定，改对反而可能红"）——线 B 统一 citable 锚口径（`anchor_is_citable` 要求非空 `sourceQuote`）后，三个测试的 fixture 锚全是"只有定位字段"的畸形形态：`integrity_report_d2_e2e` anchorsMissing 期 1 实 2、`knowledge_auto_verify_enforce` 证据判空致 needsHumanAudit=0、`sr132_review_queue` source_orphan 期 1 实 3。因 integration job 非 PR 必需检查、本地磁盘约束不跑全量，一路漏到 main 才暴露。修复=测试对齐新口径（fixture 补 `sourceQuote` 成真可引用锚；integrity_report 顺势新增畸形锚显式用例把收紧语义锁进断言）。三测试本地 Docker 全绿。**教训入档**：动了共享谓词（如锚口径）后，合并前应本地抽跑受影响域的 integration 单文件——非必需检查不会替你拦。

**完善波（2026-08-14 深夜，主会话直做，等 CI 期间）**：三件低危高值收尾。① **run_envelope 终态吸收 CAS**（28 号 §4#14 / [C] 疑点关闭）：`update_run_envelope_terminal` filter 加合法转移谓词（allowed_from 由 `is_valid_lifecycle_transition` 纯函数派生——单一真相源；legacy 无 lifecycle 键行与空串残行放行）；CAS miss 补查区分"行存在被拒"（fail-soft 落 `run_envelope_lifecycle_transition_rejected` 审计事件、整条 $set 不落防部分污染）与"行不存在"（原 insert 兜底）；`assert_lifecycle_valid` 改用新 `ALL_LIFECYCLES` 常量消除双清单；TDD 红→绿（迟到覆写 completed 现被拒 + legacy 行回归保护，`run_envelope_integration` 7/0）。与用户 WIP 把 lifecycle 当验收契约用的方向呼应。② **hold 请示零台账补审计**（28 号 §4#15 前半 / [C] 疑点关闭）：`escalate_held_decision` 骚扰门拦截分支落 `escalation_suppressed_by_push_policy` 事件（dedupe_key=contact+epoch 日，每客户每 UTC 日一条防刷，best-effort 不阻断 hold 主链）；:676 超时改派侧亲验无此问题（pending 行已存在，被拦仅延迟）。③ **macOS 爆栈家族统一处置**（线 A/D 登记项关闭）：新增 `.cargo/config.toml` `[env] RUST_MIN_STACK=16MiB`（虚拟地址预留），run_envelope_integration 7/0、principal_decision_channel 16/0、outbox_integration 23/0 均无需 env 前缀。验证：lib 2573/0（+1 系用户 WIP 演进）、`-D warnings`、禁词 0、evolution 隔离绿。
...
**重要待重验疑点 [C]**（子代理发现、主会话未逐条抽查，引用前必须重验）：~~hold 请示被骚扰门拦时零台账~~（已修 2026-08-14 完善波：拦截时落 dedupe 审计事件，每客户每 UTC 日一条）；~~delivery_unknown 请示卡不进超时改派静默滞留~~（已修，见上）；~~run_envelope 终态写无 lifecycle CAS~~（已修 2026-08-14 完善波：终态吸收 CAS + rejected 审计事件 + legacy 行放行，TDD 红→绿）；prompt shadow 真实 LLM 消耗不计入 EvolutionBudget；quiet-hours runtime 加载粒度 contact/workspace 分裂；biz-test domain8 `severity="BLOCKED"` 误用致降级分支死代码（用户 biz-test 活跃开发区，不越界）；~~`.env.example` 缺 POST_DECISION/SILENCE_SIGNAL 两族变量~~（已修 2026-08-14 D/E 尾波）；~~登录限流不解析 XFF~~（裁决为有意设计：`.env.example:230-233` 明示只信直连 TCP peer、多副本要求边缘独立限流）；两处 bson DateTime 扩展 JSON wire 残留；~~死路由 tripwire 名单缺 11 文件~~（已修 2026-08-14 D/E 尾波，实缺 12）。
