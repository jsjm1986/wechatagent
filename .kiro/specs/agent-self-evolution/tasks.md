...
> **✅ Done Notice (2026-05-27)**：W0 → W4 + 收口全部已落地，详见 `git log --oneline | grep '^.* M4 W'`。
...
> `m009_prompt_template_versioned`。
>
> **Phase A 弃用项（2026-05-26 commit `031d442`）**：原 W4 Task 5.9 的 4 个 testcontainer 集成测试
> （`tests/evolution_{isolation,prompt_e2e,rollback,threshold_e2e}.rs`）+ Task 4.8 的
> `tests/evolution_significance_pbt.rs` 在 sales-domain 收敛过程中被一并删除（依赖
> 5 闸命中率 / 销售域 fixture）。后续若要恢复这部分覆盖，需基于 3 闸 + wiki 知识库
> 重写 cohort fixture，不能直接复活旧测试。
>
> **CI 已接入（2026-05-27）**：`scripts/check-evolution-isolation.sh` 与
> `scripts/check-no-model-hint.sh` 已加入 `.github/workflows/ci.yml` baseline gate。
...
- [x] 1. 基础设施波（W0）：collection、索引、配置、CI lint
  - [x] 1.1 在 `src/db/mod.rs` 新增 4 个 collection accessor
  ...
  - [x] 1.3 在 `src/config.rs` 扩展 `AppConfig`，新增 14 条演化器字段
  ...
  - [x] 1.4 新建 CI lint 脚本 `scripts/check-evolution-isolation.sh` + `.ps1`
  ...
  - [x] 1.5 一次性迁移：`prompt_templates` 升级为多版本形态
...
- [x] 2. 骨架波（W1）：worker 主循环 + EvolutionBudget + experiment 信封 + cohort 选择
  - [x] 2.1 新建 `src/evolution/mod.rs`，实现 `pub async fn run_evolutionary_worker(state: AppState)`
  ...
  - [x] 2.2 新建 `src/evolution/budget.rs`，实现 `EvolutionBudget`
  ...
  - [x] 2.3 实现 `experiment` 信封写入与状态推进 helper
  ...
  - [x] 2.4 新建 `src/evolution/cohort.rs`，实现 cohort 选择
  ...
  - [x] 2.5 主循环骨架接通：tick 内调用 envelope + cohort，写最小事件
...
- [x] 3. 候选生成波（W2）：threshold 候选 + Critic LLM prompt 候选
  - [x] 3.1 新建 `src/evolution/threshold.rs`，实现纯统计阈值候选
  ...
  - [x] 3.2 在 `src/prompts.rs` 添加 `ensure_evolution_prompt_pack_v1`
  ...
  - [x] 3.3 新建 `src/evolution/prompt_critic.rs`，实现 Critic LLM 调用
  ...
  - [x] 3.4 新建 `src/evolution/lint.rs`，运行时禁词扫描
  ...
  - [x] 3.5 把 W2 接入主循环，update experiment.budget_used
...
- [x] 4. Shadow eval + 显著性波（W3）
  - [x] 4.1 新建 `src/evolution/replay.rs`，实现"模拟 gateway"
  ...
  - [x] 4.2 把 `src/agent/` 内 helper 暴露面最小化扩 `pub(crate)`
  ...
  - [x] 4.4 新建 `src/evolution/significance.rs`，纯函数显著性测试
  ...
  - [x] 4.8 为 `significance.rs` 写 PBT（新文件，不计 baseline 4 PBT）
...
- [x] 5. Release + 前端 + 回滚波（W4）
  - [x] 5.1 实现 `resolve_thresholds` 集中读路径
  ...
  - [x] 5.2 新建 `src/evolution/release.rs`，实现 release_threshold / release_prompt
  ...
  - [x] 5.3 实现 `prompt_pack_version` cache 失效机制
  ...
  - [x] 5.4 实现 rollback_threshold / rollback_prompt
  ...
  - [x] 5.5 新建 `src/routes/evolution.rs`，实现 4 个路由
  ...
  - [x] 5.6 新建 `src/evolution/post_release.rs`，+24h 对比窗口评测
  ...
  - [x] 5.7 新建 `frontend/src/EvolutionCenterTab.tsx`
  ...
  - [x] 5.8 在 `App.tsx` 顶层 channel/tab 状态机注册 EvolutionCenterTab
    - 与 `AutonomyOutcomesTab` 同级
    - tab 文案"演化中心"
    - `evolution_enabled=false` 时 Tab 仍显示但内容是"演化器未启用"占位（仅 admin 可见）
    - _Requirements: 7.1, 9.1_

  - [x] 5.9 为 W4 写 lib 集成测试（4 例，testcontainers，`#[ignore]`）
...
- [x] 6. 收口：监控、CI、文档、baseline
  ...
  - [x] 6.3 更新 `docs/architecture.md` + `docs/data-and-api.md`
  ...
  - [x] 6.6 端到端手工烟雾（`EVOLUTION_ENABLED=true` 临时打开）
...
- [x] 演化器关停时不影响主进程（`EVOLUTION_ENABLED=false` 单测覆盖）
- [x] `src/evolution/` 不引用 `src/agent/gateway / outbox / src/mcp`（CI lint 覆盖）
- [x] 所有新事件 kind / 前端文案过禁词 lint
- [x] release 与正在进行 run 不竞争（resolve_thresholds 在 run 入口取一次）
- [x] rollback 后 resolve_thresholds 读回老值（集成测试覆盖）
- [x] prompt_pack_version bump 后 LRU cache 失效（集成测试覆盖）
- [x] shadow replay 100 次后 outbox / conversation_messages outbound size 不变（集成测试覆盖）
- [x] admin auth middleware 复用，不引入新 token 路径
- [x] Critic LLM prompt 自身不进入 prompt evolution 循环（W2 任务 3.2 内置黑名单）
