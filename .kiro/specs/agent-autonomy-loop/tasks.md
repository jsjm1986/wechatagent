...
- [~] 1. 基础设施波（W0）：新建 collection、索引、迁移脚手架与 runtime 参数
  - [~] 1.1 在 `src/db/mod.rs` 新增 3 个 collection accessor
  ...
  - [~] 1.3 在 `src/models.rs` 与 `src/config.rs` 扩展 `RuntimeParameters`
  ...
  - [~] 1.4 在 `src/db/migrations.rs` 注册 3 条迁移脚手架（仅占位，不写实质 logic）
...
- [~] 2. 协议骨架波（W1）：删除 Rust 业务兜底、引入 Raw 双层结构、Run Envelope
  - [~] 2.1 删除 `src/agent/guards.rs::normalize_decision_runtime` 中"补默认值"逻辑
  ...
  - [~] 2.2 在 `src/agent/types.rs` 新增 `RawAgentDecision` 反序列化结构
  ...
  - [~] 2.3 实现 `RawAgentDecision::validate_and_promote(self, runtime) -> (AgentDecision, Vec<String>)`
  ...
  - [~] 2.4 新建 `src/agent/run_envelope.rs` 模块
  ...
  - [~] 2.5 把 `write_agent_run_log` 改写为 `update_one`，禁止 re-insert
...
- [~] 3. 校验 + 最终安全门波（W2）：finalize_review_for_send / local_decision_review 二态 / R5 verified knowledge
  - [~] 3.1 改造 `src/agent/review.rs::local_decision_review` 为二态
  ...
  - [~] 3.2 实现 `finalize_review_for_send` 最终安全汇总层
  ...
  - [~] 3.3 在 `src/agent/types.rs::DecisionReviewResult` 扩字段并校验枚举
  ...
  - [~] 3.4 在 gateway 主路径接入 `finalize_review_for_send`
  ...
  - [~] 3.5 实现 R5 verified knowledge 强约束细节
  ...
  - [~] 3.6 为 finalize_review_for_send 写 lib 单元测试（≥ 7 例）
  ...
  - [~] 3.7 为 R2 revision 控制流写 lib 单元测试（≥ 5 例）
  ...
  - [~] 3.8 检查点 — W2 校验/安全门波收尾
    - Ensure all tests pass, ask the user if questions arise.

- [~] 4. 工具循环 + 字典波（W3）：reply_with_tools_loop + MCP knowledge.* + system_taxonomies
  - [~] 4.1 在 `src/agent/budget.rs` 扩展 `RunBudget`
  ...
  - [~] 4.2 实现 MCP knowledge.* 三个工具的派发函数
  ...
  - [~] 4.3 实现 `reply_with_tools_loop` 多轮派发循环
  ...
  - [~] 4.6 新建 `src/agent/taxonomy.rs` 模块
  ...
  - [~] 4.7 在 `enforce_decision_guards` 中接入 taxonomy
  ...
  - [~] 4.9 实现 `2026_05_006_taxonomy_seed` 迁移
  ...
  - [~] 4.10 为字典 / candidate 写 lib 单元测试（≥ 4 例）
  ...
  - [~] 4.16 检查点 — W3 工具/字典波收尾
    - Ensure all tests pass, ask the user if questions arise.

- [~] 5. 发送闭环波（W4）：outbox 模块 + dispatcher worker + 二次安全门 + 取消通道
  - [~] 5.1 新建 `src/agent/outbox.rs` 模块与数据结构
  ...
  - [~] 5.2 实现 `OutboxDispatcher` worker（atomic claim + lease）
  ...
  - [~] 5.5 改造 gateway 主路径：决策落地改走 outbox enqueue
  ...
  - [~] 5.8 为 outbox 写集成测试（testcontainers，≥ 6 例）
...
- [~] 6. MemoryCard 边界替换波（W5）：整层 typed + stable id + deprecatedFacts
  - [~] 6.1 把 `OperatingMemory.memory_card` 整层从 `Document` 改为 `MemoryCardTyped`
  ...
  - [~] 6.2 实现 `MemoryFact` 强类型与 `MemoryFactRepr` 反序列化兼容
  ...
  - [~] 6.3 改造 memory 模块所有 helper 改用 typed
  ...
  - [~] 6.4 实现 `apply_consolidator_deprecations` 处理 deprecatedFacts / conflicts
  ...
  - [~] 6.5 实现 conflicts 事件与 Reply Agent context 注入
  ...
  - [~] 6.6 实现 `2026_05_005_memory_facts_to_structured` 迁移
  ...
  - [~] 6.7 处理写入路径自动转换 + 前端 MemoryCardSummary 展示
  ...
  - [~] 6.8 扩展 `tests/memory_card_invariants.rs` PBT
  ...
  - [~] 6.9 为 R7 deprecatedFacts 写 lib 单元测试（≥ 4 例）
  ...
  - [~] 6.11 检查点 — W5 MemoryCard 边界替换波收尾
    - Ensure all tests pass, ask the user if questions arise.

- [~] 7. 监控 + PBT 收口波（W6）：前端自治 Tab + outcomes API + happy_path 扩展 + sunset 文档 + CI lint
  - [~] 7.1 实现 `GET /api/outcomes/autonomy` 后端聚合接口
  ...
  - [~] 7.4 扩展 `tests/happy_path_run.rs` 端到端冒烟
  ...
  - [~] 7.8 兜底 lib 单元测试 — autonomy_mode 落库
  ...
  - [~] 7.9 最终检查点 — 全部 PBT + lib + 集成 + happy_path 通过 + 基线脚本通过
    - Ensure all tests pass, ask the user if questions arise.
