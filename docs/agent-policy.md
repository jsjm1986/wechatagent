...
## 演进路线 changelog（Phase 0 → Phase E3）
...
- 删除 `agent::knowledge_tools` / `agent::tool_loop` 整体（user-ops 入口零引用）；保留 `chat_tool_loop` 给 admin chat。
...
- `lessons_learned` collection（跨用户聚合）：`feedback_worker` 14d 滑窗周期归纳 success / reviewer_misjudge_negative / blocked_by_safety_guard 三类模式；upsert 到 `lessons_learned`，由 admin 决定是否晋升为 `peer_case` chunk（不绕开 review queue）。

### Phase E：通用性抽象

**已落地：**

- `Contact::locale: Option<String>` + `PromptTemplate::locale: Option<String>`（BCP-47 短形式）；`load_prompt_for_contact` 优先选同 locale 的 active 模板，未命中 fallback 到 `DEFAULT_LOCALE`（`zh-CN`），再未命中 fallback 到 `default_prompt_content`。R11 兼容：旧 contact / 旧模板缺字段反序列化为 None，由 `contact_locale_or_default` / `template_locale_or_default` 透明回退。

**留待第二个 domain（group / moments）真实落地驱动：**

- `trait OpsDomain`：domain-agnostic 框架，user-ops 为第一实现。
- `trait LlmProvider`：OpenAI / DeepSeek / Qwen 多模并行 reviewer 双模 epistemic distance。
- MCP 工具动态注册：`McpClient::list_tools` + 白名单审计 + sandbox 审核位。
- 业务规则版本化：`operation_state_policies / operation_domain_configs / system_taxonomies` 加 `active_versions`（复用 Phase C 的 contact-hash 桶逻辑）。

CLAUDE.md "Don't add features beyond what the task requires" 约束下，单 domain 期不预先做框架抽象，避免 trait 边界写错。当 group / moments 真实落地、产生第二个 domain 调用方时再启动 E1 / E2 / E4 / E5。
