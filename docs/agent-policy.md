...
- **E2-T1：LlmProvider trait + OpenAI 包装**：`src/llm_provider.rs` 抽出 `trait LlmProvider`（`generate_json` / `embed`），现有 OpenAI / DeepSeek 客户端为第一实现；`generate_agent_json` 的 LRU 缓存与 `RunBudget` 计费路径不变，未来扩第二/第三 provider 不动主干。
- **E2-T2：reviewer 双模并行 + 分歧触发 single-shot revision**：`review::review_decision` 在 `REVIEWER_DUAL_MODEL_ENABLED=true` 时把 reviewer 输入并行喂两路 LlmProvider（默认 primary + cross_provider），分歧（评分差 ≥ 阈值或 grounding/hallucination 决策不一致）时触发一次 revision，达成 epistemic diversity；同模无分歧时只走 primary，不增预算。
...
- `trait OpsDomain`：domain-agnostic 框架，user-ops 为第一实现，`decision / gateway / knowledge_router` 按 domain 分发。
- MCP 工具动态注册：`McpClient::list_tools` + 白名单审计 + sandbox 审核位。

CLAUDE.md "Don't add features beyond what the task requires" 约束下，单 domain 期不预先做 `OpsDomain` trait 抽象，避免 trait 边界写错。当 group / moments 真实落地、产生第二个 domain 调用方时再启动剩余 E 任务。
