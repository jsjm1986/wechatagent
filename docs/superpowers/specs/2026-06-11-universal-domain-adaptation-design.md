...
**日期**：2026-06-11
**状态**：实施中 — **Phase 0 已提交（commit e93cd63）；Phase 1 子步 1B 已提交（557cb38）、1D 已提交（a36164d）、1A 已提交（a40c6ab）、1C 已提交（20288c3）、1E 已提交（1b95d5a）；1F 拆为 1F-a（H8 运营范式，已落 working tree 待提交）+ 1F-b（H19 作息门控，待实施）**；后续子步 1G 待实施
**作者**：运营 agent 侧
**目标读者**：产品负责人 + 知识库后端维护者
...
- **1F · H8 运营范式可配化（§3.6）** —— 拆为 1F-a（H8 planner 范式）+ 1F-b（H19 作息门控）两个独立 commit：
  ...
  - **1F-b · H19 作息门控纳入范式链**（待实施）：`quiet_hours` 从全域单值改为可被 contact/范式覆盖（情感陪伴"晚上黄金时段"不被 22→8 压制），纳入 `resolve_operation_mode`/override 链；涉及 gateway/webhooks 送信路径的 quiet-hours 判定点。DEFAULT 等价。
