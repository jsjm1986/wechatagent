...
**第三闸：LLM 红线语义审查（堵插入型绕过，用户 2026-06-26 决策加入第一期）**。双闸只查"删没删锚 / 有没有字面禁词"，挡不住**插入型语义绕过**——保留全部锚段、不用任何字面禁词，但插入语义等价的违规话术（如"遇到拿不准的我转给后台的专业老师帮你跟进"——无字面"人工/接管"，实质却是承认真人后台 + 承诺转交，正是 :1000/:1023 红线所禁）。故在双闸之后加第三闸：

- **只审 diff 增量**：对比写回内容与当前库内 content，LLM judge 只判**新增/改动部分**是否"变相引入真人接管 / 削弱 grounding / 绕过 AI 永不自动 verify"。审增量比审整篇好判、省 token。
- **三态结局**（关键，避免 fail-closed/fail-open 两难）：
  1. 跑通判干净 → 放行（已过前两闸）。
  2. 跑通判违规 → **拒写**，把违规理由返给管理者；管理者看到理由后可带 `force=true` 强制覆盖（防 LLM 误判挡正常编辑），但必须先看到"为什么判违规"。
  3. 重试退避后仍不可用（端点 503/空/超时/不可解析）→ **降级人确认**：返回 diff + "LLM 审查没跑通，需你逐字核对有无变相接管"，人确认后才写。
- **降级人确认 = 汇流回项目主干**：LLM 审查失败不是死路，而是把把关者从机器降级到人——永远有一道语义防线，既不挡死正常编辑（非 fail-closed），也不放水（非 fail-open）。与项目"提议→人确认→执行"主干同构。
- **先重试退避再降级**：本项目端点历史 503/空常见但重试基本能过；瞬时抖动靠重试兜掉，真挂了才降级人确认。

**两条编辑路径统一**（防 dual-path drift）：判定函数 `review_prompt_edit(template_key, old, new) -> PromptEditVerdict`（三态：`Pass` / `Reject(reason)` / `NeedsHumanConfirm{diff, reason}`）单点产出，两个消费端复用同一判定：
- **路径A**（管理 agent 工具 `edit_prompt_template`）：走 command run，`NeedsHumanConfirm` 天然复用 §5.2 confirm 循环。
- **路径B**（管理员前端 prompt 编辑器直接调 REST `PUT /prompt-templates/:id`）：`NeedsHumanConfirm`/`Reject` 时后端返回需二次确认的响应，前端弹框显示 diff + 风险提示，管理者勾选"我已逐字核对"后带 `force=true` 重提。**两条路径都有人确认兜底，体验一致**（用户决策）。

> 字面双闸（禁词 + 锚完整）仍是前置硬门，快、确定、无依赖、两路都先过；LLM 第三闸是其后的语义增强。第一期 verify 类工具的恒确认（§4.3）与本第三闸是两套独立防线（一个守知识 verify，一个守 prompt 编辑）。
...
## 8. 部署 + 真实大模型全量冒烟（验收，用户 2026-06-26 决策）

完整计划落地后，部署到生产服务器用真实大模型跑全量冒烟，作为本次做厚的验收。**冒烟范围 = 管理 agent 链路（不真发微信消息）**（用户决策）：

- **需真 LLM**：build_management_plan（管理对话出 plan）和提示词第三闸（LLM 红线语义审查）都真调大模型，冒烟必须用真 key、真端点，不能 mock。
- **不需真 MCP 发送 / 真微信号**：不验证 webhook→客户对话→真发消息这条客户侧链（那是另一条已跑通的链）。MCP key 只要能让进程启动即可。
- **验收必过项**：①服务起得来（无启动 panic，特别是空状态机 active 的历史坑）；②管理对话真调通 LLM 出合理 plan + readonly 工具真执行；③提议→确认→执行循环闭合 + outcome 如实核实；④**提示词三闸真拦**（字面双闸删锚/禁词被拒 + LLM 第三闸真判变相接管 + LLM 挂时降级人确认 + 正常编辑放行）——这是红线验证的核心。
- **环境**：服务器 / 端口 / LLM 端点 key / MCP key 由用户开工前提供，凭据绝不进 git（.env chmod 600，会话内注入 DEPLOY_PASS）。详见实现计划 Task 9。
- 实现见 `docs/superpowers/plans/2026-06-26-management-agent-thickening.md` Task 9。
